...
// Fetch users and their monthly targets for the current month
$current_month = date('Y-m-01');
$sales_users_stmt = $conn->prepare("
    ...
    LEFT JOIN daily_sales ds ON u.id = ds.user_id AND ds.date >= :current_month
    WHERE (u.role = 'sales' OR u.role = 'salesadmin') 
    AND (u.active = 1 
        OR (u.active = 0 
            AND u.deactivated_at >= :current_month 
            AND u.deactivated_at < DATE_ADD(:current_month, INTERVAL 1 MONTH)
            AND ds.date >= u.deactivated_at
        )
    )
    ORDER BY u.name ASC
");
  ...
  <div class="content">
    ...
    <?php foreach ($sales_users as $user): ?>
        <tr>
            ...
            <td><input type="number" name="sales_data[<?php echo $user['id']; ?>][active_leads]" class="form-control"  value="0" required></td>
            <td><input type="number" name="sales_data[<?php echo $user['id']; ?>][active_quotes]" class="form-control" value="0" required></td>
            <td><input type="number" name="sales_data[<?php echo $user['id']; ?>][awaiting_docs]" class="form-control" value="0" required></td>
            <td>
    <input type="number" name="sales_data[<?php echo $user['id']; ?>][monthly_target]" 
           ...
           <?php echo $user['monthly_target'] > 0 ? 'readonly' : ''; ?>>
            ...
            <td><input type="number" name="sales_data[<?php echo $user['id']; ?>][daily_target]" class="form-control" value="0" required></td>
            <td><input type="number" name="sales_data[<?php echo $user['id']; ?>][daily_sale]" class="form-control" value="0" required></td>
