...
$selected_role = 'sales';
$selectedUser = 'all'; // Initialize the variable

// Handle form submission for role change
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $selected_role = $_POST['role'] ?? 'sales';
    $selectedUser = $_POST['userId'] ?? 'all'; // Handle selected user
}

// Fetch users based on the selected role
$usersQuery = "SELECT id, name FROM users WHERE role = :role ORDER BY name";
$usersStmt = $conn->prepare($usersQuery);
$usersStmt->execute([':role' => $selected_role]);
...
$salesQuery = "
    SELECT u.name, 
           ...
           (SELECT SUM(ds2.sales)
            ...
            AND ds2.date BETWEEN :startDate AND :endDate) AS current_monthly_sale
    FROM daily_sales ds
    ...
    AND (
        u.active = 1 
        OR (u.active = 0 AND u.deactivated_at >= :startDate AND u.deactivated_at < DATE_ADD(:startDate, INTERVAL 1 MONTH))
    )
    GROUP BY u.id, u.name
    ORDER BY u.name
";

// Modify the query to filter by selected user if not 'all'
if ($selectedUser !== 'all') {
    $salesQuery .= " AND u.id = :selectedUser";
}
...
// Calculate totals
$totalSales = $totalTargets = $totalActiveLeads = $totalActiveQuotes = $totalAwaitingDocs = $totalMonthlyTargets = $totalCurrentMonthlySales = 0;
...
<script src="script.js?v=1.0.1"></script>
  ...
  <body class="">
   ...
   <form id="export-form" action="export_report.php" method="post" style="display:inline;">
    ...
    <select id="role-dropdown" name="role" onchange="this.form.submit();">
        ...
        <tbody>
            <?php if (!empty($salesData)): ?>
                ...
                <tr style="font-weight: bold;">
                    ...
                    <td><?php echo htmlspecialchars($totalTargets); ?></td>
                    <td><?php echo htmlspecialchars($totalActiveLeads); ?></td>
                    <td><?php echo htmlspecialchars($totalActiveQuotes); ?></td>
                    <td><?php echo htmlspecialchars($totalAwaitingDocs); ?></td>
                    <td><?php echo htmlspecialchars($totalMonthlyTargets); ?></td>
                    <td><?php echo htmlspecialchars($totalCurrentMonthlySales); ?></td>
                </tr>
            <?php else: ?>
                <tr>
...
document.getElementById('role-dropdown').addEventListener('change', function () {
    ...
    var salesAdminTable = document.getElementById('sales-admin-table');

    if (selectedRole === 'sales') {
        salesTable.style.display = 'block';
        salesAdminTable.style.display = 'none';
    } else {
        salesTable.style.display = 'none';
        salesAdminTable.style.display = 'block';
    }
});
</script>
