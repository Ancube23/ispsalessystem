...
$selected_role = isset($_GET['role']) ? $_GET['role'] : 'sales'; // Default to 'sales' if no role is selected
...
if ($selected_role === 'sales') {
    $stmt = $conn->prepare("
        ...
        ORDER BY u.name ASC
    ");
} else if ($selected_role === 'salesadmin') {
    $stmt = $conn->prepare("
        SELECT u.id, u.name, COALESCE(mt.monthly_target, 0) AS monthly_target
        FROM users u
        LEFT JOIN monthly_targets mt ON u.id = mt.user_id AND mt.month = :current_month
 ...
 <style>
  ...
  <div class="content">
    ...
    <button type="submit" class="btn btn-primary" style="width: 13%; padding: 6px 12px;">Add Sales</button>
                   ...
                   <table class="table table-bordered" tyle="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr>
