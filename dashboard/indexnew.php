...
try {
    ...
    SELECT 
        ...
        COUNT(CASE WHEN u.role != 'admin' AND u.active = 1 THEN u.id END) AS total_users,
    ...
    WHERE u.role != 'admin' AND u.active = 1"; // Ensure only active users are included
    ...
    WHERE ds.date BETWEEN :start_date AND :end_date
      AND u.active = 1"; // Ensure only active users are included
    ...
    $allUsersQuery = "SELECT COUNT(*) AS all_users FROM users WHERE role != 'admin' AND active = 1"; // Ensure only active users are included
    ...
    // Get total users
    $totalUsersQuery = "SELECT COUNT(*) AS total_users FROM users WHERE active = 1"; // Ensure only active users are included
...
WHERE u.role IN ('sales', 'salesadmin')
  AND u.active = 1  -- Ensure only active users
  AND (u.deactivated_at IS NULL OR u.deactivated_at > :end_date)  -- Exclude users deactivated before the current period
GROUP BY u.id, u.name, u.role";

// Query to get only active sales users
$activeSalesUsersQuery = "
    SELECT u.name, COALESCE(SUM(ds.sales), 0) AS sales, COALESCE(SUM(ds.target), 0) AS target
    FROM users u
    LEFT JOIN daily_sales ds ON ds.user_id = u.id AND ds.date BETWEEN :start_date AND :end_date
    WHERE u.role = 'sales' AND u.active = 1  -- Only active sales users
    GROUP BY u.id, u.name";

$activeSalesUsersStmt = $conn->prepare($activeSalesUsersQuery);
$activeSalesUsersStmt->bindParam(':start_date', $startDate);
$activeSalesUsersStmt->bindParam(':end_date', $endDate);
$activeSalesUsersStmt->execute();
$activeSalesUsersData = $activeSalesUsersStmt->fetchAll(PDO::FETCH_ASSOC);


// Query to get only active sales admin users
$activeSalesAdminQuery = "
    SELECT u.name, COALESCE(SUM(ds.sales), 0) AS sales, COALESCE(SUM(ds.target), 0) AS target
    FROM users u
    LEFT JOIN daily_sales ds ON ds.user_id = u.id AND ds.date BETWEEN :start_date AND :end_date
    WHERE u.role = 'salesadmin' AND u.active = 1  -- Only active sales admin users
    GROUP BY u.id, u.name";

$activeSalesAdminStmt = $conn->prepare($activeSalesAdminQuery);
$activeSalesAdminStmt->bindParam(':start_date', $startDate);
$activeSalesAdminStmt->bindParam(':end_date', $endDate);
$activeSalesAdminStmt->execute();
$activeSalesAdminData = $activeSalesAdminStmt->fetchAll(PDO::FETCH_ASSOC);




// Query to get deactivated users (active = 0 or deactivated_at is not NULL)
$deactivatedUsersQuery = "
    SELECT u.name, u.role, COALESCE(SUM(ds.sales), 0) AS sales, COALESCE(SUM(ds.target), 0) AS target
    FROM users u
    LEFT JOIN daily_sales ds ON ds.user_id = u.id
    WHERE u.active = 0 OR u.deactivated_at IS NOT NULL
    GROUP BY u.id, u.name, u.role";

$deactivatedStmt = $conn->prepare($deactivatedUsersQuery);
$deactivatedStmt->execute();
$deactivatedUsersData = $deactivatedStmt->fetchAll(PDO::FETCH_ASSOC);
   ...
   $salesUsersData = array_filter($salesUsersData, function($user) {
    return isset($user['deactivated_at']) && $user['deactivated_at'] === null;  // Ensure deactivated_at is checked properly
});

$salesAdminsData = array_filter($salesAdminsData, function($user) {
    return isset($user['deactivated_at']) && $user['deactivated_at'] === null;  // Ensure deactivated_at is checked properly
});
    ...
    // Ensure only active users who are not deactivated or deactivated after the current period
$salesUsersData = array_filter($salesUsersData, function($user) use ($endDate) {
    return $user['active'] == 1 && (is_null($user['deactivated_at']) || $user['deactivated_at'] > $endDate);
});

$salesAdminsData = array_filter($salesAdminsData, function($user) use ($endDate) {
    return $user['active'] == 1 && (is_null($user['deactivated_at']) || $user['deactivated_at'] > $endDate);
});
...
<script src="script.js?v=1.0.1"></script>
  ...
  <body class="">
    ...
    <tbody id="salesTableBody">
        ...
        <?php else: ?>
            <?php foreach ($salesUsersData as $user): ?>
                <?php if ($user['active'] == 1): // Only show active users ?>
                    <tr>
    ...
    <div class="container mt-4">
    <center><h4><b>Active Sales Users</b></h4></center>
    <table class="table table-striped">
        <thead class="text-primary">
            <tr>
                <th>Name</th>
        ...
        <tbody id="activeSalesUsersTableBody">
            <?php if (empty($activeSalesUsersData)): ?>
                <tr>
                    <td colspan="3" class="text-center">No active sales users available</td>
                </tr>
            <?php else: ?>
                <?php foreach ($activeSalesUsersData as $user): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($user['name']); ?></td>
                        <td><?php echo htmlspecialchars($user['sales']); ?></td>
                        <td><?php echo htmlspecialchars($user['target']); ?></td>
...
<div class="container mt-4">
    <center><h4><b>Active Sales Admin Users</b></h4></center>
    <table class="table table-striped">
        <thead class="text-primary">
            <tr>
                <th>Name</th>
        ...
        <tbody id="activeSalesAdminTableBody">
            <?php if (empty($activeSalesAdminData)): ?>
                <tr>
                    <td colspan="3" class="text-center">No active sales admin users available</td>
                </tr>
            <?php else: ?>
                <?php foreach ($activeSalesAdminData as $user): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($user['name']); ?></td>
                        <td><?php echo htmlspecialchars($user['sales']); ?></td>
                        <td><?php echo htmlspecialchars($user['target']); ?></td>
    ...
    <div class="container mt-4">
       <center><h4><b>Inactive</b></h4></center>
    ...
    <tbody id="deactivatedUsersTableBody">
        <?php if (empty($deactivatedUsersData)): ?>
            <tr>
                <td colspan="3" class="text-center">No deactivated users available</td>
            </tr>
        <?php else: ?>
            <?php foreach ($deactivatedUsersData as $user): ?>
                <tr>
                    <td><?php echo htmlspecialchars($user['name']); ?></td>
                    <td><?php echo htmlspecialchars($user['sales']); ?></td>
                    <td><?php echo htmlspecialchars($user['target']); ?></td>
