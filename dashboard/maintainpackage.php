...
// Get available subgroups for the selected FNO
$subgroups = [];
if (isset($_POST['fno_id']) && $_POST['fno_id'] !== "") {
    $fno_id = $_POST['fno_id'];
    $stmt = $pdo->prepare("SELECT subgroup_id, subgroup_name FROM fno_subgroup WHERE fno_id = :fno_id");
    $stmt->execute(['fno_id' => $fno_id]);
    $subgroups = $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Fetch FNOs
$fno_stmt = $pdo->query("SELECT FNO_id, FNO_Name FROM fno");
$fno_list = $fno_stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch packages with extended search functionality
$search_term = isset($_GET['search']) ? $_GET['search'] : ''; // Search term input from the user
$fno_filter = isset($_GET['fno_filter']) ? $_GET['fno_filter'] : '';
$pricelist_filter = isset($_GET['pricelist_filter']) ? $_GET['pricelist_filter'] : '';

$query = "SELECT p.PK_ID, f.FNO_Name, pl.PR_Short_Description, p.PK_Term, p.PK_Speed, p.PK_Price, s.subgroup_name 
    FROM package p
...
if (!empty($search_term)) {
    // Adding LIKE filters for multiple columns
    $search_term = "%" . $search_term . "%"; // Prepare the search term for LIKE
    $conditions[] = "(f.FNO_Name LIKE :search_term 
                      OR pl.PR_Short_Description LIKE :search_term 
                      OR CAST(p.PK_Speed AS CHAR) LIKE :search_term
                      OR CAST(p.PK_Price AS CHAR) LIKE :search_term
                      OR CAST(p.PK_Term AS CHAR) LIKE :search_term
                      OR s.subgroup_name LIKE :search_term)";
}
...
try {
    ...
    logError("PDO Error during query execution: " . $e->getMessage());
...
<div class="mb-3">
    ...
    <select name="subgroup_id" id="subgroupSelect" class="form-control">
        ...
        <?php foreach ($subgroups as $subgroup): ?>
            <option value="<?= $subgroup['subgroup_id']; ?>"><?= htmlspecialchars($subgroup['subgroup_name']); ?></option>
        <?php endforeach; ?>
    </select>
</div>
    ...
    <div class="card shadow">
        <div class="card-header bg-dark text-white">
            ...
            <form method="GET" id="filterForm" class="d-flex">
                <input type="text" name="search" id="search" class="form-control" placeholder="Search packages..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
                <button type="submit" style="border: none; background: transparent; padding: 0; margin-left: -250px;">
                    <img src="./img/search.png" alt="Search" style="width: 30px; height: 30px;">
                </button>
            </form>
        </div>
                    ...
                    <tbody>
                        ...
                        try {
                            ...
                            $query = "
                                ...
                                LEFT JOIN fno_subgroup s ON p.subgroup_id = s.subgroup_id
                                WHERE (f.FNO_Name LIKE :search OR pr.PR_Short_Description LIKE :search)
                            ";

                            // Add filters to the query if they are set
                            if ($fno_filter) {
                                $query .= " AND p.FNO_id = :fno_filter";
                            }
                            if ($pricelist_filter) {
                                $query .= " AND p.PR_id = :pricelist_filter";
                            }
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"></script>
    ...
    <script>
    // Event listener for FNO dropdown change
    document.getElementById("fnoSelect").addEventListener("change", function() {
        ...
        if (fnoId) {
            // Send AJAX request to fetch subgroups using Fetch API
            fetch(`fetch_subgroups.php?fno_id=${fnoId}`)
                .then(response => response.json())
                .then(data => {
                    data.forEach(subgroup => {
                        let option = document.createElement("option");
                        option.value = subgroup.subgroup_id; // Ensure `subgroup_id` is sent as per your DB schema
                        option.textContent = subgroup.subgroup_name;
    ...
    // Existing script for handling FNO and Pricelist filters
    ...
    $("#updatePackageBtn").click(function() {
        ...
        $.ajax({
            url: "update_package.php",  // Create this file to handle the update
            type: "POST",
            data: {
                id: id,
                term: term,
    ...
    $("a[data-toggle='modal']").click(function() {
        ...
        $("#confirmDeleteBtn").off('click').on('click', function() {
            $.ajax({
                url: "delete_package.php",  // Create this file to handle the deletion
                type: "POST",
