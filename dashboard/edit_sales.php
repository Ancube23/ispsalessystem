...
// Prepare the query to fetch sales data
$sales_stmt = $conn->prepare("
    ...
    AND (
        u.active = 1 
        OR (u.active = 0 
            AND u.deactivated_at >= :first_day_of_month 
            AND u.deactivated_at < DATE_ADD(:first_day_of_month, INTERVAL 1 MONTH)
            AND ds.date >= u.deactivated_at
            AND MONTH(u.deactivated_at) = MONTH(:first_day_of_month)
            AND YEAR(u.deactivated_at) = YEAR(:first_day_of_month)
        )
    )
