...
// Fetch only active users with the role "sales" for the dropdown
$usersQuery = "SELECT id, name FROM users WHERE role = 'sales' AND active = 1 ORDER BY name"; // Only active users
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch sales data for the selected user or all users (only "sales" role users)
    if ($selectedUser !== 'all') {
        $salesQuery = "SELECT u.name, 
                              ...
                              COALESCE(MAX(mt.monthly_target), 0) AS monthly_target,
                       ...
                       LEFT JOIN monthly_targets mt ON ds.user_id = mt.user_id AND mt.month = :currentMonthStart
    ...
    <?php if (!empty($salesData)): ?>
        ...
        <?php foreach ($salesData as $row): ?>
            <?php if (!in_array($row['id'], $displayedUserIds)): ?>
                ...
                <?php 
                // Add the user ID to the displayed list
        ...
        <tr style="background-color: lightgray;">
            ...
            <td><strong><?php echo htmlspecialchars($totalTargets); ?></strong></td>
            <td><strong><?php echo htmlspecialchars($totalActiveLeads); ?></strong></td>
            <td><strong><?php echo htmlspecialchars($totalActiveQuotes); ?></strong></td>
            <td><strong><?php echo htmlspecialchars($totalAwaitingDocs); ?></strong></td>
            <td><strong><?php echo htmlspecialchars($totalMonthlyTargets); ?></strong></td>
            <td><strong><?php echo htmlspecialchars($totalCurrentMonthlySales); ?></strong></td>
        </tr>
    <?php else: ?>
        <tr>
