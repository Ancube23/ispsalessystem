...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($sales_data as $sales_data_item) {
        ...
        if (empty($sales_data_item['user_id'])) {
    ...
    if ($all_fields_filled) {
        foreach ($sales_data as $sales_data_item) {
            // Check if user_id is set and not empty
            if (empty($sales_data_item['user_id'])) {
                continue; // Skip this entry if user_id is missing
            }

            $sales_user_id = $sales_data_item['user_id'];
            $daily_sale = isset($sales_data_item['daily_sale']) ? $sales_data_item['daily_sale'] : null;
            $daily_target = isset($sales_data_item['daily_target']) ? $sales_data_item['daily_target'] : null;
            $active_leads = isset($sales_data_item['active_leads']) ? $sales_data_item['active_leads'] : null;
            $active_quotes = isset($sales_data_item['active_quotes']) ? $sales_data_item['active_quotes'] : null;
            $awaiting_docs = isset($sales_data_item['awaiting_docs']) ? $sales_data_item['awaiting_docs'] : null;
            $monthly_target = isset($sales_data_item['monthly_target']) ? $sales_data_item['monthly_target'] : null;
            ...
            try {
                ...
                if ($monthly_target !== null) {
                    ...
                    $monthly_target_stmt->bindParam(':monthly_target', $monthly_target, PDO::PARAM_INT);
                ...
                $error = "Failed to update sales data: " . $e->getMessage();
            }
        }
    } else {
        $error = "At least the user_id must be provided for each entry";
    }
}
