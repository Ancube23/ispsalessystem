...
// Cache-control headers to ensure the latest data is loaded
...
$role_filter = 'sales'; // Default role
$current_month = date('Y-m-01'); // Initialize current_month
...
$sales_users_stmt = $conn->prepare("
    ...
    ORDER BY u.name ASC
");
$sales_users_stmt->execute(['current_month' => $current_month]);
$sales_users = $sales_users_stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch sales admin users
$sales_admin_users_stmt = $conn->prepare("
    SELECT u.id, u.name, COALESCE(mt.monthly_target, 0) AS monthly_target
    FROM users u
    LEFT JOIN monthly_targets mt ON u.id = mt.user_id AND mt.month = :current_month
    ...
    ORDER BY u.name ASC
");
$sales_admin_users_stmt->execute(['current_month' => $current_month]);
$sales_admin_users = $sales_admin_users_stmt->fetchAll(PDO::FETCH_ASSOC);
?>
 ...
 <style>
  ...
  <div class="content">
    ...
    <div class="form-group">
        ...
        <select name="role" id="role_select" class="form-control2">
            <option value="sales" <?php echo $role_filter == 'sales' ? 'selected' : ''; ?>>Sales</option>
            <option value="salesadmin" <?php echo $role_filter == 'salesadmin' ? 'selected' : ''; ?>>Sales Admin</option>
        </select>
    </div>
</form>
<div class="table-container">
    <div id="sales-table" class="table-wrapper active">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <?php endforeach; ?>
                    ...
                    <div id="sales-admin-table" class="table-wrapper">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th>Name</th>
            ...
            <tbody>
                <?php foreach ($sales_admin_users as $user): ?>
                    <tr>
                        <td><?php echo $user['name']; ?></td>
                        <!-- Other columns as before -->
                    </tr>
    ...
    <button id="toggle-button" class="btn btn-primary">Show Sales Admin</button>
</div>
            </div>
        </div>
    </div>
    ...
    const salesAdminTable = document.getElementById('sales-admin-table');

    toggleButton.addEventListener('click', function() {
        salesTable.classList.toggle('active');
        salesAdminTable.classList.toggle('active');
        
        // Toggle button text
        if (salesAdminTable.classList.contains('active')) {
            toggleButton.textContent = 'Show Sales';
        } else {
            toggleButton.textContent = 'Show Sales Admin';
        }
    });
});
