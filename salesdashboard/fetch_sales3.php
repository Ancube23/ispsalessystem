...
try {
    ...
    // Fetch sales and target data for the past 7 days
    $endDate = date("Y-m-d"); // Today's date
    $startDate = date("Y-m-d", strtotime("-7 days")); // 7 days before today
...
$htmlContent .= '</div>';
    $htmlContent .= "<p><center>Hi {$userName},<br><br>You made {$totalSales} sales in the past 7 days with an expected target of {$totalTarget}.</center></p>";
    $htmlContent .= '</div>';
